...
class Board
  ...
  def store_current
    ...
    # for each block in piece: get [x, y]coords of block, set on grid the new position of the block after move
    (0..3).each{|index| 
  ...
  def remove_filled
    (2..(@grid.size-1)).each{|num| row = @grid.slice(num);
      ...
      if @grid[num].all?
        ...
        ((@grid.size - num + 1)..(@grid.size)).each{|num2| # for all rows on top of the removed row
          @grid[@grid.size - num2].each{|rect| rect && rect.move(0, block_size)}; # move row blocks one step down
          @grid[@grid.size-num2+1] = Array.new(@grid[@grid.size - num2]) # create an empty row on top of the row that moved down
        }
