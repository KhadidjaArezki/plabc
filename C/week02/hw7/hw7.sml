...
datatype geom_exp = 
   NoPoints
	 ...
	 | Var of string
(* CHANGED add shifts for expressions of the form Shift(deltaX, deltaY, exp *)
...
(* CHANGED: Add a case for Shift expressions *)
      | Shift (deltaX, deltaY, exp) => let val res = eval_prog (exp, env) in
  (case res of
      NoPoints => NoPoints
    ...
    | _ => raise BadProgram("Cannot shift a non-value geometric expression"))
  end
...
fun preprocess_prog e = 
  case e of 
    LineSegment (x1, y1, x2, y2) =>
      let 
        val zero_deltaX = real_close(x1, x2)
        val zero_deltaY = real_close(y1, y2)
      in 
        if zero_deltaX
        then
          if zero_deltaY
